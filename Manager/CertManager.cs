...
namespace Manager
{
	public class CertManager
	{
        ...
        public static string GetCertificateGroupFromStorage(StoreName storeName, StoreLocation storeLocation, string groupName)
        {
            X509Store store = new X509Store(storeName, storeLocation);
            store.Open(OpenFlags.ReadOnly);

            X509Certificate2Collection certCollection = store.Certificates.Find(X509FindType.FindBySubjectName, groupName, true);

            /// Check whether the subjectName of the certificate is exactly the same as the given "subjectName"
            //Debugger.Launch();
            foreach (X509Certificate2 c in certCollection)
            {
                string grupa = c.SubjectName.Name;
                string spltiString = grupa.Split(',')[1];
                string grp = spltiString.Split('=')[1];
